...
## 四、如果只剩下 2 个 Sentinel，会发生什么？

这种情况下：

- 多数派是 `⌊2/2⌋ + 1 = 2`
    
- 如果 1 台宕机，只剩 1 台活着 → 无法形成多数派 → 无法进行 Failover。
    

**结果：不会发生自动切换。**

> Sentinel 会继续等待更多节点恢复（或者人工介入）。

也就是说：

> Sentinel 必须有 **多数派存活** 才能进行故障转移。

---

## 五、举例：3 哨兵选举过程时序图

`时间轴 → ┌────────────────────────────────────────────────────────────┐ │ A检测主挂 → 进入SDOWN                                     │ │ B检测主挂 → 进入SDOWN                                     │ │ C检测主挂 → 进入SDOWN                                     │ │ A向B、C发起is-master-down-by-addr(epoch=20)               │ │ B还没投票过 → YES投给A                                    │ │ C还没投票过 → YES投给A                                    │ │ A获得2票（A+B or A+C） → 成为Leader                       │ │ A执行failover                                              │ │ B、C接收通知，更新主节点信息                              │ └────────────────────────────────────────────────────────────┘`
