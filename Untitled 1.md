...
def GIT_URL = 'https://gitlab.datatist.cn/datatist/wolf-app-static'
def PACKAGE_DIR= '/workspace/package'
def ARTIFACT_ID = 'wolf-app-static'
def NAMESPACE = 'dev-wolf'
...
def NODE_VERSION_DICT=["v12.22":"/root/node-v12.22.12-linux-x64/bin/", "v14.18":"/root/node-v14.18.0-linux-x64/bin/", "v16.13.0":"/root/node-v16.13.0-linux-x64/bin/"]
...
pipeline {
    ...
    parameters {
        ...
        choice(name: 'node_version', choices: ['v16.13.0'], description: 'node版本')
        ...
        choice(name: 'apps', choices: ['ALL', 'evaluation', 'wolf-app-base-static'], description: '打包应用') 
    }
    stages {
        stage('Git Checkout') {
            steps {
        ...
        stage('Build 指标') {
            when { expression { return params.apps == 'ALL' || params.apps == 'evaluation' } }
            steps {
                sh """
                ...
                if [[ ${params.need_build} == "true" ]];then
                  cd Evaluation  && export PATH=/root/node-v16.13.0-linux-x64/bin:$PATH && npm install -g pnpm@7 && pnpm --version && pnpm install && pnpm run build
                fi
                """
            }
        }
        stage('Build wolf-app-base-static') {
            when { expression { return params.apps == 'ALL' || params.apps == 'wolf-app-base-static' } }
            steps {
                sh """
                node_version_tag="${params.node_version}"
                export PATH="$NODE_VERSION_PATH:$PATH"
                ...
                git --version 
                if [[ ${params.need_build} == "true" ]];then
                  cd wolf-app-base-static  && export PATH=/root/node-v16.13.0-linux-x64/bin:$PATH && npm install -g pnpm@7 && pnpm --version && pnpm install && pnpm run build
                fi
                """
            }
        }
        ...
        stage ('Deploy') {
            steps {
                sh """
                    ...
                    apps=(Evaluation wolf-app-base-static)
                    ...
                    if [ \"${params.restart_application}\" == \"true\" ]; then
                        kubectl patch -n ${NAMESPACE} deployment wolf-app-static --type='json' -p '[{"op": "replace", "path": "/spec/template/spec/containers/1/args/0", "value": "'\${final_file_suffix}'" }]'
                        sleep 2
                        pods=`kubectl get pod -n ${NAMESPACE} -l name=wolf-app-static | sed -e 1d | awk '{print \$1}'`
                        [ -n "\$pods" ] && echo \$pods | xargs kubectl delete pod -n ${NAMESPACE}
                        echo "success deploy wolf-app-static \${final_file_suffix}" 
                    else
                        echo "skip deploy wolf-app-static \${final_file_suffix}" 
                    fi
                    # 是否需要将包发布至ftp
